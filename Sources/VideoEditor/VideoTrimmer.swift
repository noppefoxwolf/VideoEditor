...
final class VideoTrimmer: UIControl {
    ...
    func setupAsset(_ asset: AVAsset) async throws {
    ...
    var maxDuration: CMTime = .positiveInfinity
    ...
    private func locationForTime(_ time: CMTime) -> CGFloat {
        ...
        guard visibleRange.duration.seconds.isNormal else { return 0 }
        let visibleDurationInSeconds = CGFloat(visibleRange.duration.seconds)
        let ratio = visibleDurationInSeconds != 0 ? availableWidth / visibleDurationInSeconds : 0
    ...
    @objc private func leadingGrabberPanned(_ sender: UILongPressGestureRecognizer) {
        ...
        case .changed:
            ...
            if CMTimeCompare(newDuration, maxDuration) == 1 {
                time = CMTimeSubtract(selectedRange.end, maxDuration)
                didClamp = true
            }
    ...
    @objc private func trailingGrabberPanned(_ sender: UILongPressGestureRecognizer) {
        ...
        case .changed:
            ...
            if CMTimeCompare(newDuration, maxDuration) == 1 {
                time = CMTimeAdd(selectedRange.start, maxDuration)
                didClamp = true
            }
            if CMTimeCompare(time, range.start) == -1 {
                time = range.start
    ...
    override func layoutSubviews() {
        ...
        Task {
            try await regenerateThumbnailsIfNeeded()
